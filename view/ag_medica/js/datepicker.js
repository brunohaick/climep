 ...
 * 
 * This project is distinct from and not affiliated with the jquery.ui.calendar.
 * 
...
(function ($) {
  var DatePicker = function () {
    var ids = {},
      views = {
        years: 'calendarViewYears',
        moths: 'calendarViewMonths',
        days:  'calendarViewDays'
      },
      tpl = {
        wrapper: '<div class="calendar"><div class="calendarBorderT" /><div class="calendarBorderB" /><div class="calendarBorderL" /><div class="calendarBorderR" /><div class="calendarBorderTL" /><div class="calendarBorderTR" /><div class="calendarBorderBL" /><div class="calendarBorderBR" /><div class="calendarContainer"><table cellspacing="0" cellpadding="0"><tbody><tr></tr></tbody></table></div></div>',
        head: [
          '<td class="calendarBlock">',
          '<table cellspacing="0" cellpadding="0">',
            '<thead>',
              '<tr>',
                '<th colspan="7"><a class="calendarGoPrev" href="#"><span><%=prev%></span></a>',
                '<a class="calendarMonth" href="#"><span></span></a>',
                '<a class="calendarGoNext" href="#"><span><%=next%></span></a></th>',
              '</tr>',
              '<tr class="calendarDoW">',
        ...
        space : '<td class="calendarSpace"><div></div></td>',
        days: [
      ...
      defaults = {
        ...
        current: null,
        /**
         * true causes the calendar calendar to be appended to the DatePicker 
         ...
         * 
         * @param HTMLDivElement el the calendar containing element, ie the 
         *        div with class 'calendar'
        ...
        onChange: function() { },
        /**
         * Invoked before a non-inline calendar is shown, with 'this'
         * referring to the HTMLElement that DatePicker was invoked upon, ie
         * the trigger element
         * 
         * @param HTMLDivElement el The calendar container element, ie the div with class 'calendar'
         * @return true to allow the calendar to be shown, false to keep it hidden
         */
        onBeforeShow: function() { return true },
        /**
         * Invoked after a non-inline calendar is shown, with 'this'
         * referring to the HTMLElement that DatePicker was invoked upon, ie
         * the trigger element
         * 
         * @param HTMLDivElement el The calendar container element, ie the div with class 'calendar'
        ...
        /**
         * Invoked before a non-inline calendar is hidden, with 'this'
         * referring to the HTMLElement that DatePicker was invoked upon, ie
         * the trigger element
         * 
         * @param HTMLDivElement el The calendar container element, ie the div with class 'calendar'
         * @return true to allow the calendar to be hidden, false to keep it visible
         */
        onBeforeHide: function() { return true },
        /**
         * Invoked after a non-inline calendar is hidden, with 'this'
         * referring to the HTMLElement that DatePicker was invoked upon, ie
         * the trigger element
         * 
         * @param HTMLDivElement el The calendar container element, ie the div with class 'calendar'
       ...
       * 
       * @param HTMLDivElement el calendar container element
       */
      fill = function(el) {
        var options = $(el).data('calendar');
        var cal = $(el);
        ...
        for(var i = 0; i < options.calendars; i++) {
          ...
          if(i == 0) tblCal.addClass('calendarFirstView');
          if(i == options.calendars - 1) tblCal.addClass('calendarLastView');
          
          if(tblCal.hasClass('calendarViewDays')) {
            dow = date.getMonthName(true)+", "+date.getFullYear();
          } else if(tblCal.hasClass('calendarViewMonths')) {
            dow = date.getFullYear();
          } else if(tblCal.hasClass('calendarViewYears')) {
          ...
          data = {
            data: [],
            className: 'calendarYears'
          }
          for( var j = 0; j < 12; j++) {
            data.data.push(dow + j);
          }
          // calendarYears template
          ...
          while(cnt < 42) {
            ...
            if (today.getDate() == date.getDate() && today.getMonth() == date.getMonth() && today.getYear() == date.getYear()) {
              data.weeks[indic].days[indic2].classname.push('calendarToday');
            }
            if (date > today) {
              // current month, date in future
              data.weeks[indic].days[indic2].classname.push('calendarFuture');
            }
            
            if (month != date.getMonth()) {
              data.weeks[indic].days[indic2].classname.push('calendarNotInMonth');
              // disable clicking of the 'not in month' cells
              data.weeks[indic].days[indic2].classname.push('calendarDisabled');
            }
            if (date.getDay() == 0) {
              data.weeks[indic].days[indic2].classname.push('calendarSunday');
            }
            if (date.getDay() == 6) {
              data.weeks[indic].days[indic2].classname.push('calendarSaturday');
            }
            ...
            if(options.date && (!$.isArray(options.date) || options.date.length > 0)) {
              if (fromUser.selected || options.date == val || $.inArray(val, options.date) > -1 || (options.mode == 'range' && val >= options.date[0] && val <= options.date[1])) {
                data.weeks[indic].days[indic2].classname.push('calendarSelected');
              }
            }
          ...
          // Fill the calendarDays template with data
      ...
      layout = function(el) {
        ...
        }).find('div.calendarContainer').css({
          width: width + 'px',
          height: height + 'px'
        });
      },
      ...
      click = function(ev) {
        ...
        if (el.is('a')) {
          ev.target.blur();
          if (el.hasClass('calendarDisabled')) {
            return false;
          }
          var options = $(this).data('calendar');
          ...
          if (parentEl.is('th')) {
            ...
            if (el.hasClass('calendarMonth')) {
              ...
              if(options.mode == 'range') {
                ...
                if(tblEl.eq(0).hasClass('calendarViewDays')) {
                  tblEl.eq(0).toggleClass('calendarViewDays calendarViewMonths');
                  el.find('span').text(tmp.getFullYear());
                } else if(tblEl.eq(0).hasClass('calendarViewMonths')) {
                  tblEl.eq(0).toggleClass('calendarViewMonths calendarViewYears');
                  el.find('span').text((tmp.getFullYear()-6) + ' - ' + (tmp.getFullYear()+5));
                } else if(tblEl.eq(0).hasClass('calendarViewYears')) {
                  tblEl.eq(0).toggleClass('calendarViewYears calendarViewDays');
              ...
              if(tblEl.eq(0).hasClass('calendarViewDays')) {
                options.current.addMonths(el.hasClass('calendarGoPrev') ? -1 : 1);
              } else if(tblEl.eq(0).hasClass('calendarViewMonths')) {
                options.current.addYears(el.hasClass('calendarGoPrev') ? -1 : 1);
              } else if(tblEl.eq(0).hasClass('calendarViewYears')) {
                options.current.addYears(el.hasClass('calendarGoPrev') ? -12 : 12);
              }
              fillIt = true;
            }
            
          } else if (parentEl.is('td') && !parentEl.hasClass('calendarDisabled')) {
            // clicking the calendar grid
            
            if(tblEl.eq(0).hasClass('calendarViewMonths')) {
              // clicked a month cell
              options.current.setMonth(tblEl.find('tbody.calendarMonths td').index(parentEl));
              options.current.setFullYear(parseInt(tblEl.find('thead th a.calendarMonth span').text(), 10));
              options.current.addMonths(currentCal - tblIndex);
              tblEl.eq(0).toggleClass('calendarViewMonths calendarViewDays');
            } else if(tblEl.eq(0).hasClass('calendarViewYears')) {
              // clicked a year cell
              options.current.setFullYear(parseInt(el.text(), 10));
              tblEl.eq(0).toggleClass('calendarViewYears calendarViewMonths');
            } else {
              // clicked a day cell
                var val = parseInt(el.text(), 10);
                tmp.addMonths(tblIndex - currentCal);
                if (parentEl.hasClass('calendarNotInMonth')) {
       ...
       * can be called directly to show the bound calendar.  A DatePicker
      ...
      hide = function (ev) {
        if (ev.target != ev.data.trigger && !isChildOf(ev.data.cal.get(0), ev.target, ev.data.cal.get(0))) {
          if (ev.data.cal.data('calendar').onBeforeHide.apply(this, [ev.data.cal.get(0)]) != false) {
            ev.data.cal.hide();
            ev.data.cal.data('calendar').onAfterHide.apply(this, [ev.data.cal.get(0)]);
    ...
    return {
      ...
      init: function(options){
        ...
        return this.each(function(){
          if (!$(this).data('calendar')) {
            ...
            var id = 'calendar_' + parseInt(Math.random() * 1000), cnt;
            options.id = id;
            $(this).data('calendarId', options.id);
            var cal = $(tpl.wrapper).attr('id', id).bind('click', click).data('calendar', options);
      ...
      /**
       * Returns the currently selected date(s) and the calendar element.
       * 
      ...
      getDate: function() {
        if (this.size() > 0) {
          return prepareDate($(this).data('calendar'));
        }
      },
